...
class CoreAttention(MegatronModule):
    ...
    def flash_attention(self, query_layer, key_layer, value_layer, attention_mask, attention_bias, inference_mode):
        ...
        if attention_bias is not None:
            ...
            return self.flash_attention_cuda_concat_batch(query_layer, key_layer, value_layer, attention_mask, is_causal,)
    ...
    def flash_attention_cuda_concat_batch(self, query_layer, key_layer, value_layer, attention_mask, is_causal, cu_seqlens_q=None, cu_seqlens_k=None):
        """
        Run flash_attn_unpadded_func for concatenated batch samples, given seq_offsets.

        Args:
            query_layer: of shape [1, concatenated_len_q, num_heads_q, head_dim]
            key_layer:   of shape [1, concatenated_len_kv, num_heads_kv, head_dim]   
            value_layer: of shape [1, concatenated_len_kv, num_heads_kv, head_dim] 
            cu_seqlens:  of shape [1, batch_size+1]

        Returns:
            context_layer: of shape [1, concatenated_len_q, num_heads_q, head_dim] 
        """
        batch_size = 1

        q = rearrange(query_layer, "b s ... -> (b s) ...")
        k = rearrange(key_layer, "b s ... -> (b s) ...")
        v = rearrange(value_layer, "b s ... -> (b s) ...")
        ...
        # [1, sq, np, hn]
        context_layer = rearrange(context_layer, "(b s) ... -> b s ...", b=batch_size)

        # [1, sq, np, hn] -> [1, np, sq, hn]
        context_layer = context_layer.permute(0, 2, 1, 3)
        return context_layer
